...
# Results -> results/urbansarfloods/lp.csv ; per-run viz -> results/urbansarfloods/<features>_<head>_wce.png
#
...
#
#   sbatch scripts/slurm/urbansarfloods/sweep.sh          # batch (recommended)
#   bash   scripts/slurm/urbansarfloods/sweep.sh          # or interactively inside a GPU salloc
...
CSV=results/urbansarfloods/lp.csv
...
for TILE in 16 32 64; do
  echo "=== TILE $TILE: prep ==="
  python -u -m exp.urbansarfloods.prep_tiles --splits train,valid --tile_size "$TILE"
done
...
for TILE in 16 32 64; do
  for PS in ${PATCHES[$TILE]}; do
    ...
    python -u -m exp.urbansarfloods.extract_features --splits train,valid --tile_size "$TILE" --patch_size "$PS"

    for HEAD in concat diff; do
      echo "=== LP $FEAT head=$HEAD ==="
      python -u -m exp.urbansarfloods.lp --features "$FEAT" --head "$HEAD" \
        --weighted_ce --results_csv "$CSV"
    done
  done
done

echo "=== DONE. Results: $CSV ; visualizations: results/urbansarfloods/ ==="
