...
# scripts/slurm/pastis/lp_heads.sh. Run from a login node (NOT under sbatch):
#   ./scripts/slurm/pastis/submit_lp_heads.sh
# Override the feature list by passing them as arguments:
#   ./scripts/slurm/pastis/submit_lp_heads.sh oe_base_s2_ps1_tile1 oe_base_s2s1_ps4_tile64
#
# Gate every LP job on an extraction job finishing successfully (afterok). Pass the
# extraction job id via DEPEND, e.g.:
#   DEPEND=4011001 ./scripts/slurm/pastis/submit_lp_heads.sh oe_base_s2_ps1_tile1
...
for feat in "${FEATURES[@]}"; do
    ...
    sbatch "${DEP_ARG[@]}" --export=ALL,FEATURES="$feat" scripts/slurm/pastis/lp_heads.sh
done
