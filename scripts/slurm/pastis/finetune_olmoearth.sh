...
python -c "import sys,json; from exp.common.config import load_config, to_dict; ov=[a for a in sys.argv[1:] if '=' in a]; print(json.dumps(to_dict(load_config(None, ov)), indent=2))" "$@" \
