...
# The feature set is passed via the FEATURES env var (use scripts/slurm/pastis/submit_lp_heads.sh to fan out one
# job per feature set), e.g.:
#   sbatch --export=ALL,FEATURES=oe_base_s2_ps1_tile1 scripts/slurm/pastis/lp_heads.sh
#
...
if [ -z "$FEATURES" ]; then
    echo "ERROR: set FEATURES, e.g. sbatch --export=ALL,FEATURES=oe_base_s2_ps1_tile1 scripts/slurm/pastis/lp_heads.sh" >&2
    exit 1
fi
