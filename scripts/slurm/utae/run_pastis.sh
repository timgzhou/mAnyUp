...
source env_setup/env.sh

# Email the resolved config at start (also validates required fields before training).
python -c "import sys,json; from dataclasses import asdict; from exp.utae.run_pastis import load_config; ov=[a for a in sys.argv[1:] if '=' in a]; print(json.dumps(asdict(load_config(None, ov)), indent=2))" "$@" \
    | mail -s "[START job $SLURM_JOB_ID] $*" "$EMAIL"

LOG="logs/utae_pastis_${SLURM_JOB_ID}.out"
python -u -m exp.utae.run_pastis "$@"
STATUS=$?
