...
same kernel and optimizer, only the guidance band count differs.
    ...
    source env_olmo.sh        # AnyUp pulls torch.hub; UPA/UPMA need cuda
    python compare_upa_anyup_oeps1.py
    python compare_upa_anyup_oeps1.py --img_idx 3 --guide_bands all
"""
import argparse
import math
import sys
...
def main():
    ...
    p.add_argument("--repo", type=Path, default=Path(__file__).resolve().parent,
                   help="repo root (holds olmo_shims/ and data/)")
    ...
    p.add_argument("--out", type=Path, default=Path("compare_upa_anyup_oeps1.png"))
    args = p.parse_args()
...
# python -u compare_upa_anyup_oeps1.py