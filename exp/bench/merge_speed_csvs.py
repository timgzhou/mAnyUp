...
def main() -> None:
    ...
    try:
        running = subprocess.run(["squeue", "-h", "-n", "bench", "-o", "%i"],
                                 capture_output=True, text=True, timeout=30).stdout.split()
    except Exception:
        running = []
    if running and os.environ.get("MERGE_ANYWAY") != "1":
        raise SystemExit(
            f"{len(running)} bench job(s) still running ({', '.join(running)}); their CSVs are "
            f"mid-write and merging now would drop rows.\n"
            f"Wait for them, or set MERGE_ANYWAY=1 to override.")

    parts = sorted(glob.glob("results/pastis/bench/olmoearth_ps-tile_speed_*_img*.csv"))
    rows = []
    for p in parts:
    ...
    dest = "results/pastis/bench/olmoearth_ps-tile_speed.csv"
