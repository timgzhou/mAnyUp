...
from PASTIS and they change the experiment:
  ...
  4. 3 classes (non-flood / flooded-open / flooded-urban), and the headline mIoU covers only
     classes 0/1 -- matching lp_urbansarfloods.py, where flooded-urban is reported separately.
...
UPA/UPMA are imported from compare_upa_anyup_oeps1.py (single source of truth).
...
Prerequisites (neither is checked into this repo -- build them first):
    python prep_urbansarfloods_tiles.py
    python extract_urbansarfloods_features.py --splits train,valid
"""
import argparse
import csv
import json
...
from compare_upa_anyup_oeps1 import UPA, UPMA, percentile_stretch

RESULTS_CSV = "results/upsampler_usf.csv"
...
# UrbanSARFloods S1 tile band layout (prep_urbansarfloods_tiles.py, and the viz in
# lp_urbansarfloods.py:151): 8 bands, of which the intensity bands are
...
def main():
    ...
    sys.path.insert(0, str(args.repo / "olmo_shims"))
    import olmo_bootstrap
    olmo_bootstrap.apply()

    from lp_urbansarfloods import (TiledFeatureDataset, NUM_CLASSES, IGNORE_LABEL,
    ...
    if not feat_dir.exists():
        raise SystemExit(
            ...
            f"  python prep_urbansarfloods_tiles.py\n"
            f"  python extract_urbansarfloods_features.py --splits train,valid")
    ...
    if not tiles_dir.exists():
        raise SystemExit(f"no tiles at {tiles_dir} (needed for SAR guidance); run "
                         f"prep_urbansarfloods_tiles.py")
